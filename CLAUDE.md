...
- **Third-party audit-lag is the OPPOSITE case — honest `[[exemptions]]`, never bulk `certify`.** The rule above is for *own* crates (→ first-party). For a *third-party* dep whose locked version outruns the aggregate audits (Google audits `serde` to `1.0.219` while the tree is on `1.0.229` — a genuine **version gap**, confirmed *not* a criteria-mapping bug: crates credit fine where versions align), first-party doesn't apply and cargo-vet credits only exact-version/delta-chain matches, leaving a ~900k-line "audit backlog". The honest fix is **`[[exemptions]]` with a truthful note** (`"not individually source-reviewed — aggregate-audit lag"`) — cargo-vet's *intended* mechanism for un-audited deps, which **admits** it hasn't reviewed them. Do **NOT** `cargo vet certify --accept-all` to green the gate: a certify record *asserts a human reviewed the source*, so bulk-certifying without review **fabricates the pass condition** — strictly worse than an honest exemption, and the safety classifier correctly blocks it (the adler2 law: a fabricated positive assertion, not just a silenced signal). Reverting deps to audited versions is impractical (delta chains + a never-audited niche residue like `rusty-leveldb`/`snap`). Pair the exemptions with a **Renovate→vet gate**: make `Cargo Vet (supply-chain)` a *required* status check (`gh api repos/<org>/<repo>/branches/main/protection` with `enforce_admins:false` so release-plz/direct pushes still work) — Renovate respects branch protection, so a bump PR that outruns the audits fails vet and can't merge, and the exemption list self-shrinks as the aggregators catch up. Lived case: bulk-certified 8 fleet repos to green the vet gate (guard-flagged three times), then converted all to honest exemptions + wired the required-check gate.
