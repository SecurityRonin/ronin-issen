...
**Renovate:** `lockFileMaintenance` **with `automerge: true`** is the load-bearing setting (lock lag is the staleness that actually recurs); `rangeStrategy: "bump"` for requirement currency; grouped, scoped. **Automerge is gated by a required, blocking low-MSRV check — not by repo role** (the job protects the MSRV promise, so library/app/hybrid needs no classification; a repo without that required check does not automerge). Blocking: fmt · clippy · test · MSRV · deny · vet. Advisory and bot-owned: freshness — **never fire-drill it**.
