...
- **Standalone executables for macOS (aarch64 + x86_64), Linux (aarch64 + x86_64, musl-static), and Windows (x86_64 MSI)** — attached to a GitHub Release with a `checksums.txt`.
  - **GUI apps** (a repo shipping a windowed binary — e.g. timeglyph's `timeglyph-lens` overlay): the Windows **MSI must install the GUI binary AND create a Start Menu shortcut**. A portable winget **zip** gives the CLI a PATH alias but **no launcher**, so the GUI ends up *"nowhere to be found"* (lived case: winget installed only `timeglyph.exe`, never the lens). Two binaries from two crates + the ICE-clean shortcut pattern (HKCU keypath in a ProgramMenuFolder subfolder; never `light -sval`) are in the **release skill → gotcha #7 (GUI-app MSI)**. macOS/Linux currently ship the GUI as the plain **Homebrew/apt binary** (`bin.install`) — runnable from a terminal but with **no `.app`, Dock, or Launchpad presence**. The macOS analogue of the Windows Start Menu shortcut is a **Homebrew Cask**, not a Formula: a cask's `app "Foo.app"` stanza installs a `.app` bundle into `/Applications` (Launchpad/Spotlight/Finder), and its `binary` stanza can also symlink the CLI onto PATH (so one cask serves both, or keep the CLI in the Formula + a Cask for the GUI). That requires building a real `Foo.app` bundle (Info.plist + icon + the binary) plus a cask in the shared tap — the fleet doesn't do this yet. The fleet builds **no DMGs** (the release-skill DMG recipe is a generic template, not our standard; a cask ships the `.app` inside a zip or DMG). **Open gap — pursue only if asked.**
