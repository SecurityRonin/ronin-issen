...
**A fifth genuine bug, and the one with the widest blast radius.** `issen` carried `Mutex::lock().expect("mutex poisoned")` in **8 `DataSource::read_at` implementations** (qcow2, aff4, vhd, dmg, dd, vmdk, iso, vhdx). `read_at` already returns `Result<usize, RtError>` — **the failure had an error channel and was panicking past it.** A poisoned lock means an earlier read panicked while holding it; with the disk legs running under rayon, that converts *one* failed read of *one* image into a panic on **every subsequent read of that image**. Now returns `RtError::Io` naming the source that poisoned.

**The measured count was 510 findings across 60 files, not the ~28 estimated** — and the gap is a measurement artifact worth knowing. **Without `cargo clippy --keep-going`, cargo stops at the first failing crate and reports 1 finding, not 510.** It took five successive passes to reach the bottom because each fixed crate unmasked the next. Of the 510: **25 production sites** (all fixed at source, zero `#[allow]` added to a production site) and 482 test-side, closed with the sanctioned `cfg_attr(test, allow(...))` in 11 crate roots plus plain `#![allow(...)]` in 19 integration-test files, which a crate-root attribute cannot reach.

**A config trap that will bite every remaining repo.** `correctness` and `suspicious` are lint *groups*. Adding them at implicit priority 0 alongside individual `"allow"` lints also at priority 0 is a **hard error** — clippy refuses to run at all. The canonical recipe in `CLAUDE.core.md` already carries `priority = 1` on the allows; repos whose tables were written without it will fail the moment the groups are added, and the failure looks like a broken toolchain rather than a config error.
...
**A second, busier emitter was found by not stopping at the named one.** The brief named `supertimeline.rs:75`. `issen` also has `timeline_format.rs::write_csv` — the `issen timeline --format csv` path, **more used than supertimeline**. It writes through `csv::Writer`, so quoting and column structure were correct, but it applied **no formula guard** to the same evidence-derived `path` and `description` fields. Fixing only the named site would have left the busier command exposed. Both now share one guarded record builder.

Also worth recording from the same work: `EventType::Other(String)` carries a parser-supplied string and broke CSV columns exactly as `tags` did — the column-structure defect was not confined to the field that surfaced it.
...
- **79 repos clear one of our own crates through cargo-vet the wrong way** — the original figure of “10+ repos, `safe-read`” was the tail, not the head. Fleet-wide, **88 distinct fleet crates** were cleared by exemption rather than trust; the volume leaders are `forensicnomicon-core` (67 repos), `forensicnomicon` (61), `forensicnomicon-data` (61), `forensic-vfs` (33), `state-history-forensic` (29), with `safe-read` at 17 — not 10.

  **A rarer inversion in `blazehash`, and it is worse than an exemption.** `[policy.ewf] audit-as-crates-io = false` declares the crates.io crate `ewf` **first-party** — asserting our own workspace authored code we actually pull from the registry. Every other instance of this defect *understates* our posture; this one **overstates** it. (Unshippable for now: blazehash's committed `Cargo.lock` is stale enough that `cargo metadata` drops **157 packages**, so `cargo vet --locked` aborts. The lock refresh is a real dependency change and belongs in its own task.)

  **Negative result worth recording:** zero `[[audits.*]]` records exist anywhere in the fleet, so there is no `--accept-all` bulk certification to unwind — consistent with ADR-0018's note that previously bulk-certified repos were already converted to exemptions.

- *(original entry)* **10+ repos clear `safe-read` through cargo-vet the wrong way.** They carry `[[exemptions.safe-read]]` where ADR-0018 requires a `trust` entry — mechanism 2 (“ours, from crates.io → `cargo vet trust <crate> h4x0r`”), and the ADR is explicit that “reaching for a weaker one is a defect.” Affected: keychain, 4n6mount, ext4fs, iso9660, forensic-vfs-mount, forensic-vfs-engine, ntfs, udf, discord-desktop, livedisk. An exemption asserts “unreviewed, accepted anyway”; a trust entry asserts the publisher is ours. The distinction is the point of the mechanism. Cheap fleet sweep, and it should be folded into whatever PR wave migrates the hand-rolled readers.
...
**A coverage gate can report OK over a crate it measured not at all.** `cargo llvm-cov report` does **not** inherit the package selection from the preceding `cargo llvm-cov test`, and it *rejects* `--workspace` (`error: --workspace is specific to [test,nextest,…] and not supported for subcommand 'report'`). So a repo that adds a workspace member to what began as a single-package repo gets an LCOV file containing **no `SF:` record at all** for the new crate — and the line half of the gate passes over zero measured lines while the JSON/function half still sees it. That asymmetry is what keeps it quiet.

Found in `timeglyph` while adding `timeglyph-core`, and only by grepping the LCOV rather than trusting the gate's “OK”. The same root cause silently affects the bare `cargo test`/`clippy`/`fmt` commands: **with a root package present, cargo's default selection is that package alone**, so a new member is compiled as a dependency and its tests never run.

**This is likely not confined to one repo.** Any fleet repo that grew from single-package to workspace may have inherited it — the gate says OK, the member is unmeasured, and nothing surfaces the difference. Worth a targeted sweep: for each workspace repo, confirm its LCOV carries an `SF:` record per member.
